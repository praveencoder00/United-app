...
class _MachineDetailPage extends ConsumerState<MachineDetailPage> {
  ...
  Widget build(BuildContext context, ) {
    ...
    return DefaultTabController(
      ...
      length: 2,
      child: Scaffold(
        floatingActionButton:widget.initialIndex ==1 && paymentStatus =='Pending'? FloatingActionButton(
          onPressed: () async {
    ...
    await launchUrl(url, mode: LaunchMode.externalApplication);
        ...
        appBar: AppBar(
          ...
          actions: [
            ...
            GestureDetector(
              onTap: () async {
                await PaymentDialouge().addPaymentDialogue(context,widget.machineId,amountPaid,finalAmount);
              },
              child: Padding(
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text('${data.machineName} -- id:${data.machineId}', style: TextStyle(fontSize: 18)),
                      Text(
                        '${data.date.day}-${data.date.month}-${data.date.year}',
                        style: TextStyle(fontSize: 18),
                      ),
                      Text(
                        'Estimated Ammount: ${data.estimatedAmount}',
                        style: TextStyle(fontSize: 18),
                      ),
                      Text(
                        'Final Amount: ${data.finalAmount ?? 'not confrimed'}',
                        style: TextStyle(fontSize: 18),
                      ),
                      Text(
                      ...
                      GestureDetector(
                        onTap: () async {
                          if(data.status == 'Pending'){
                            await SimpleDialouges().statusChangeDialouge(context, 'On proccess',data.id!,data.customerNumber,data.machineName);
                            
                                
                          } else if( data.status == 'On proccess'){
                            if (data.finalAmount ==null) {
                              SimpleDialouges().finalAmountDialouge(context, 'Completed', data.id!);
                              return;
                              
                            }
                            if (data.amountPaid==data.finalAmount) {
                              await FirebaseFirestore.instance.collection('machines').doc(data.id).update({'paymentStatus':'Completed'});
                            }
                            await SimpleDialouges().statusChangeDialouge(context, 'Completed',data.id!, data.customerNumber ,data.machineName);
                            ...
                            children: [
                              Container(
                                decoration: BoxDecoration(
                                  ...
                                  color: data.status == 'Pending'? Colors.red: data.status == 'On proccess'? Colors.amber:Colors.green,
                                ),
                                width: 10,
                                height: 10,
                              ),
